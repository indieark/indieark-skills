...
from seedance2.project_store import ASSET_TYPES
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=CLI_NAME,
        description=(
            "Video Gen Pro director-grade video generation CLI. "
            f"Run `{CLI_NAME} setup` first to save your Volcengine Ark API key."
        ),
    )
    ...
    asset_p = sub.add_parser("asset", help="add, list, show, search, and reuse project character/scene/prop/graphic/voice assets")
    asset_sub = asset_p.add_subparsers(dest="asset_command", required=True)

    asset_add = asset_sub.add_parser("add", help="add a character, scene, prop, graphic, or voice asset")
    asset_add.add_argument("project_id")
    asset_add.add_argument("--type", choices=ASSET_TYPES, required=True)
    ...
    asset_add.add_argument("--role", help="stable creative role, e.g. protagonist or location; for voice assets, set the bound character name")
    ...
    asset_list.add_argument("--type", choices=ASSET_TYPES)
    ...
    asset_show.add_argument("--type", choices=ASSET_TYPES)
    ...
    asset_search.add_argument("--type", choices=ASSET_TYPES)
    ...
    asset_reuse.add_argument("--type", choices=ASSET_TYPES)
    ...
    callback_smoke.add_argument("--task-id", default="videogen-callback-smoke")
