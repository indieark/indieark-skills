...
> Pairs with: `interaction.md` 先判断是否追问；`scenes.md` 和 `methods.md` 承接场景与方法路由。
...
| Signal | Route |
| ------ | ----- |
| 只有自然语言创意 | 先进入 `../../video-gen-script/references/generation-workflow.md` Stage 1，输出剧本候选和规模估算 |
| 用户已选剧本或给了完整剧情 | 仍要做 `natural-language-to-shot-breakdown`：先从剧本拆出角色/场景/道具资产清单，再拆镜头、估时长，镜头引用清单里的资产 |
| 任意镜头有明确地点、空间、布光或道具关系 | 先过 `scene-card-before-storyboard`，检索或生成场景卡 |
| 任意非抽象镜头缺 `scene_ref` | 回到资产规划，不生成故事板，不写最终视频 prompt |
| 场景卡只想默认出一张单图 | 先读 `../../video-gen-assets/references/scene-sheet.md` 的 `Scene Card Routing Contract`，按镜头角度数、复用强度和空间风险选择类型 |
| 用户明确说快速抽样或绕过项目化流程 | 可以走快速路径，但仍输出简短镜头拆分，并在完成回复说明未登记完整资产 |
| 用户说「参考模式 / 我自己去 Sora\|Veo3\|... 跑 / 只要素材包 / 不要生成」 | 进入 advisor mode；读 `../../video-gen-advisor/references/advisor-mode.md`，按 `completion.md` 的 Advisor Reference Pack 模板输出参考包；不进入 API 操作分支，只允许只读 CLI 和 `--dry-run-payload` |
...
| -------------- | --------------------------- | ----------------------------------------------------- |
| 新生成视频     | `scenes.md` + `../../video-gen-director/references/director-method.md` | director prompt + selected-mode payload               |
| 复杂新生成视频 | `../../video-gen-storyboard/references/visual-storyboard.md`      | project assets + storyboard approval + traceable generation |
| 智能编排或多段连续视频 | `../../video-gen-storyboard/references/visual-storyboard.md` + `../../video-gen-assets/references/assets.md` | orchestration table + continuity ledger + per-segment storyboards + handoff prompts |
| 清楚可规划的镜头或角色/场景一致性强 | `../../video-gen-storyboard/references/visual-storyboard.md` + `../../video-gen-assets/references/assets.md` | lookup gate + 4K storyboard-first board + concise read-the-board video prompt |
| 像示例图一样需要 CUT 表、场景图、调度图和大量可读文字 | `../../video-gen-storyboard/references/visual-storyboard.md` + `../../video-gen-storyboard/references/script-composed-storyboard.md` | script split + atomic images + composed 4K storyboard poster + concise video prompt |
| 粒子/爆炸/烟雾/抽象动态为主 | `../../video-gen-director/references/director-method.md` + optional visual anchors | prompt-heavy or hybrid prompt; no storyboard means detailed per-shot video prompt |
| 按图动起来     | `api.md` + `scenes.md`      | first-frame payload                                   |
| 从首帧到尾帧   | `api.md` + `../../video-gen-director/references/director-method.md`    | first/last-frame payload                              |
| 多参考素材综合 | `api.md` + `scenes.md`      | omnireference payload                                 |
| 有声音效/说话人声/字幕/标题文字/参考音频 | `sound-design.md` + `../../video-gen-director/references/director-method.md` | audio/text brief + prompt sound section + generate_audio/reference_audio decision |
...
先读 `methods.md`，再按需加载具体方法。

| Need                                   | Method                                             |
| -------------------------------------- | -------------------------------------------------- |
| 单镜头、限制多、素材/声音/参数都要控住 | `../../video-gen-director/references/director-method.md`（三段式骨架，单一连续动作） |
| 叙事递进、短剧、广告、产品展示、起承转合 | `../../video-gen-director/references/director-method.md`（三段式骨架，时序递进写在【画面内容】里） |
| 轻量 prompt 或简单动作                 | 不加载方法文件，直接用 `../../video-gen-director/references/director-method.md` |
...
videogen asset search --query "<角色/场景/品牌/风格关键词>"
videogen asset search --type character --tag "<标签>"
videogen asset list <project-id> --query "<关键词>"
videogen asset show <project-id> <asset-id>
videogen asset reuse <target-project-id> <source-project-id> <asset-id>
```
...
- 没找到匹配资产：再创建项目、登记用户素材，或按 `../../video-gen-assets/references/assets.md` 调用外部图像工具生成后登记。
- 用户明确要求快速抽样并绕过项目库时，可以跳过检索，但完成回复要说明这是快速路径。

| Need | Route |
| ---- | ----- |
| 短剧、广告、系列内容、角色/场景一致性 | 先用 `project list` / `asset search` 检索是否已有项目和角色/场景资产；不足时读 `../../video-gen-assets/references/assets.md` 规划需要的人物/场景/故事板图，再用 `project` / `asset` / `script` / `style` / `storyboard` 登记，确认后用 `generate --project --storyboard` |
| 分多个视频生成且要求连续 | 进入同一 `project_id`，读 `../../video-gen-script/references/multi-segment-continuity.md` 写连续性台账；先用剧本把剧情节拍和镜头边界拆清楚，不把同一镜头拆成两次生成；每段用独立 `storyboard_id` 和 `video-prompt.txt`；AI 按 `continuity_mode` 判断是剧情连续但切镜头、尾帧硬衔接、`first-last` 桥接，还是同一长镜头 `extend` / 尾帧延续 |
| 用户需要先生成剧本、按规定时长拆段、每段多张分镜图片，再拼成类似影视分镜信息图 | 先用 `script set` 保存剧本；按 `../../video-gen-script/references/multi-segment-continuity.md` 拆 SD 段；读 `../../video-gen-storyboard/references/script-composed-storyboard.md` 规划 CUT 原子图和 composition manifest；外部图像工具生成原子图，脚本拼成 4K 故事板后 `storyboard add` / `approve` / `generate` |
| 用户已有故事板图和视频提示词 | 登记 `storyboard add`，再 `storyboard approve`，最后 `generate` |
| 用户明确快速抽样或绕过 | 走现有 6 个创建子命令，但保留 run artifact |

故事板路线只在它能让信息更清楚时使用。若选择故事板路线，故事板母图必须 4K 或更高，可以直接用高能力图像模型生成包含中文文字、CUT 表、场景调度说明和复杂版式的完整故事板；提示词要写清文字内容、栏目结构、字号层级、读图顺序、留白和画面对应关系。`video-prompt.txt` 只写读图规则、运动区域、约束区域、参考图编号用途、短声音 brief、说话人声/字幕策略和禁止项。故事板负责镜头顺序和运动，不单独负责人物、背景和道具细节；复杂任务生成时必须把对应角色卡、场景/背景卡作为额外 `--reference-image` 一起提交，并在 prompt 里说明主次。场景卡不能默认简单单图：单角度低复用才用基本场景卡，多镜头角度用多视角联合图，长期复用用正交四视图。需要确定性可编辑文字、指定字体/品牌版式、超密集 CUT 表或程序化复盘时，再进入脚本拼版路线。若选择 prompt-heavy 路线或没有故事板，不要为了形式强行生成九宫格或四栏；改用 `../../video-gen-director/references/director-method.md` 的详细分镜化视频 prompt，逐镜头写清视觉内容、镜头参数、音频轨道、说话人声、字幕/标题文字和转场。
...
| Stage      | Verify                                                                 |
| ---------- | ---------------------------------------------------------------------- |
| 提交前     | 路由已选定；自然语言已转剧本和镜头清单；每个非抽象镜头有 `scene_ref`；prompt 写清参考素材用途；故事板路线已同时准备故事板图、角色卡和场景/背景卡；有声任务写清声音设计或参考音频用途；复杂任务已确认 project（走故事板路线时还要确认 storyboard，导演法 prompt-heavy 路线确认最终 video-prompt.txt）；模式互斥通过；必要时已跑 `--dry-run-payload` |
...
| Change Type          | Verify                                                                    |
| -------------------- | ------------------------------------------------------------------------- |
| 文档/路由/index 改动 | `python skills/video-gen-pro/scripts/validate_skill.py`（自检 references 链接 + payload JSON） |
| CLI 行为改动         | `python -m py_compile skills/video-gen-pro/scripts/video_gen_pro.py` + `videogen --help` 冒烟 |
