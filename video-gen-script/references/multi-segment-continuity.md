...
> Avoid: 只生成单段 4-15 秒视频；先读 `../../video-gen-storyboard/references/storyboard-board.md` 决定故事板母图信息密度和版式。
> Pairs with: `../../video-gen-assets/references/setting-director.md` 做段落规划；`../../video-gen-storyboard/references/storyboard-board.md` 定义每段故事板母图；`../../video-gen-assets/references/character-sheet.md` / `../../video-gen-assets/references/scene-sheet.md` 锁定锚点；`../../video-gen-assets/references/seedance-handoff.md` 登记、确认和生成。
...
下面这份模板**只适用于故事板法**的 `video-prompt.txt`（一图控制 + 文字解说图片的简短 prompt）。走导演法的多段任务不要套用：每段最终 prompt 仍是三段式骨架（见 `../../video-gen-director/references/director-method.md`），有且只有【基础设定】【氛围与画质】【画面内容】三段——承接状态折进【画面内容】的开头与结尾、资产锁定折进【基础设定】、风格锁定靠【氛围与画质】逐字复用，不追加「这是第 X 段 / 上一段结束状态 / 本段结束状态 / 禁止」这些小节；段落编号与 `end_state` 留在 Segment Ledger。

故事板法的每段 `video-prompt.txt` 写明本段在全片中的位置：
...
- `extend`：同一长镜头继续向前时，优先用 `extend --reference-video <SD01 video url>`。本地 MP4 需要先提供可访问 URL，或按 `../../video-gen-pro/references/cli.md` 使用 `--serve-local-assets cloudflare`。

项目化故事板路径示例：

```bash
videogen storyboard add <project-id> --storyboard-id sb-sd01 --image sd01-storyboard.png --prompt sd01-storyboard-prompt.txt --video-prompt sd01-video-prompt.txt
videogen storyboard approve <project-id> sb-sd01
videogen generate --project <project-id> --storyboard sb-sd01 --duration <seconds> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait

videogen storyboard add <project-id> --storyboard-id sb-sd02 --image sd02-storyboard.png --prompt sd02-storyboard-prompt.txt --video-prompt sd02-video-prompt.txt
videogen storyboard approve <project-id> sb-sd02
...
```bash
videogen storyboard add <project-id> --storyboard-id sb-sd02 --image sd02-storyboard.png --prompt sd02-storyboard-prompt.txt --video-prompt sd02-cut-continuity-prompt.txt
videogen storyboard approve <project-id> sb-sd02
videogen generate --project <project-id> --storyboard sb-sd02 --duration <seconds> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait
```
...
| 角色漂移 | 回到 `../../video-gen-assets/references/character-sheet.md` 修 master 图，所有段落复用同一角色图 |
| 场景跳变 | 回到 `../../video-gen-assets/references/scene-sheet.md` 固定空间 DNA 和光源方向 |
