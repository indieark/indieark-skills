...
> Pairs with: `../../video-gen-pro/references/interaction.md` 做问询门；`../../video-gen-storyboard/references/visual-storyboard.md` 管项目库和视觉故事板；`../../video-gen-assets/references/assets.md` 管角色/场景资产；`../../video-gen-director/references/director-method.md` 写 prompt；`../../video-gen-pro/references/completion.md` 汇报结果。
...
- 剧本转资产清单：从选定剧本分析出本片需要的角色、场景、道具/产品、声线资产清单——读剧本就能确定有哪些角色、哪些场景、哪些关键道具和声线，这层与后面怎么分镜无关。资产清单是**可跨剧本复用的单元**。这是规划层：先把「本片需要哪些资产」从剧本拆清楚（来源是剧本，不是镜头）。至于这些资产**什么时候实际生成**，是执行节奏，两种都正式可选：① 批量预生成——开工就照清单一次性把资产做齐（复用性好、跨段一致性稳，推荐用于系列/多段或一致性要求高的任务）；② 按镜头懒生成——清单先列着，等某个镜头真的用到某资产时再去生成它（适合轻量/探索/单镜头任务，省去用不上的资产）。按任务规模选，不强制必须批量预生成。
- 剧本转镜头：把选定剧本拆成镜头清单，每个镜头至少有 `shot_id`、时长、画面目的、景别、运镜、动作、声音和文字轨道。
- 镜头引用资产：让每个镜头从资产清单里**引用**资产，形成 `shot_id -> character_refs / scene_ref / prop_refs` 引用表。镜头是资产的使用者，不是定义者。这里区分两件事：① **来源**——任何资产都必须先进剧本资产清单（哪怕是某镜头才发现需要的临时路人/道具，也先回填进清单再引用），不能绕过清单凭空冒出一个未登记资产；② **生成时机**——按镜头懒生成「清单里已规划的资产」是允许的正式做法，不算凭空新造。简言之：清单从剧本拆（来源固定），图什么时候做可早可晚（时机自由）。
...
- 下一步：<即将做什么>
- 需要你确认：<确认项或可修改项>
- 继续后会使用：<方法 reference / 外部图像工具 / videogen CLI 子命令>
```
...
如果资产规划显示任意非抽象镜头缺少 `scene_ref`，先生成或登记场景卡，不进入故事板图片生成或视频 prompt 编写。生成场景卡前必须读取 `../../video-gen-assets/references/scene-sheet.md` 的 `Scene Card Routing Contract`：每个主要场景都要先判断镜头角度数、复用强度和空间风险，再选择基本场景卡、正交四视图或多视角联合图；不能默认只生成简单场景单图。
...
- 导演流程复核：自然语言已转成选定剧本、剧本资产清单、镜头清单及资产引用表；每个非抽象镜头已有 `scene_ref` 或明确快速路径说明。
...
结果不满意时，按 `../../video-gen-pro/references/iteration.md` 只做 1-3 处定向修改；不要从自然语言创意阶段全部重来，除非用户明确要求重做。
