...
def _setup_interactive(cfg: dict) -> dict:
    sys.stderr.write(
        "seedance2 setup: leave a field blank to keep the current value.\n"
...
def cmd_doctor(args: argparse.Namespace) -> dict:
    ...
    diagnostics["checks"].append({
        ...
        "message": (
            ...
            else "missing API key; run `seedance2 setup` or set ARK_API_KEY"
        ),
    })
    ...
    if not diagnostics["ok"]:
        diagnostics["hint"] = "fix the failing checks above; rerun `seedance2 doctor`"
    return diagnostics
...
def _check_github_update() -> dict:
    ...
    repo = os.environ.get("SEEDANCE_GITHUB_REPO", GITHUB_REPO)
    api_base = os.environ.get("SEEDANCE_GITHUB_API", GITHUB_API_BASE).rstrip("/")
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": f"seedance2/{SCRIPT_VERSION}",
            },
        )
        ...
        if latest and latest != SCRIPT_VERSION:
            return {
                ...
                "message": (
                    ...
                    f"run the install script or: git pull && python scripts/seedance2_video.py"
                ),
...
def cmd_config_path(args: argparse.Namespace) -> dict:
    return {
        ...
        "config_dir_env": "SEEDANCE2_CONFIG_DIR",
    }
