...
本 skill 被触发后，除非用户明确只要解释、文档、功能介绍或使用向导，不允许跳过下面顺序。guide mode 是强制说明模式：必须先读 `references/guide-mode.md`，默认按它的 8 项结构详细说明；不调用 CLI、不构造 payload、不创建项目/资产/故事板、不要求 API key、不声明已生成结果。用户转为真实生成时再进入完整执行协议：
...
2. **使用向导** — 只问功能介绍、使用向导、支持模式、准备事项或边界时 → `references/guide-mode.md`
