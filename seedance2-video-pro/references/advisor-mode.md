...
> Pairs with: `../SKILL.md` 的 `## Execution Modes` 定义三档模式；`completion.md` 的 Advisor Reference Pack 模板是回复格式；`interaction.md` 的 Advisor Mode 越界确认处理模式切换。
...
- 如果用户在 advisor mode 中明确切到 executor mode（"开始生成 / 现在出视频 / 真实跑 / 执行 / run it"），先复述请求要求一次明确确认，再按 `SKILL.md` 的强制执行协议从头走。
...
1. 读 `interaction.md` 判断是否需要追问目标平台、画幅、时长、有声/无声、镜头数、是否长视频；advisor mode 下追问仍遵循"一次最多 3 个、只问会改变包内容的问题"。
2. 按需读 `router.md` 收束任务类型；advisor mode 下不进入 API 操作分支。
3. 按需读 `director.md` / `scenes.md` / `methods/README.md` 形成镜头方案；复杂任务可读 `visual-assets.md`、`visual-storyboard.md`、`sound-design.md`、`visual-assets/multi-segment-continuity.md` 形成完整资产 brief 和拆段建议。
4. 选择性调用允许的只读 CLI 命令：例如用 `asset search` 检索本地已有角色/场景卡，写进参考包；用 `--dry-run-payload` 落证据。
5. 按 `completion.md` 的 Advisor Reference Pack 模板渲染回复。
6. 不要做"执行前确认卡"——advisor mode 不真实执行；但要在模式越界时按 `interaction.md` 的 Advisor Mode 越界确认处理。

## Reference Pack Contents

advisor 模式必须至少给出以下七项（用户明确说"只要 prompt 不要镜头清单"等局部要求时可削减；但不能默认偷工）：

1. **镜头清单 Shot List**：每个镜头给时长、画幅、镜头运动、主体动作；多段视频说明拆段方案（按 `visual-assets/multi-segment-continuity.md` 的 Intelligent Orchestration Gate 思路判断）。
