...
> Pairs with: `storyboard-board.md` 定义故事板母图理念；`grid-storyboard.md` / `four-column-storyboard.md` 提供版式参考；`seedance-handoff.md` 负责登记、确认和视频提示词。

脚本拼版故事板不是新概念，而是故事板母图的一种生产方式。它把“图像生成”与“版式排版”拆开：先生成角色、场景、CUT 关键画面和俯视调度等原子图片，再用脚本把真实文字、时间码、对白、镜头说明、声音和色彩信息排进同一张 4K 信息图。

这条路线适合用户给出的示例：左侧或中央是 CUT 列表和分镜画面，右侧是场景俯视图，底部是光影、色板、风格和声音说明。关键优势是文本由脚本绘制，不依赖图像模型生成小字，故事板更容易被人确认，也更容易回溯。

## When To Use

Use when:

- 故事板信息密度高，需要 CUT 编号、时间码、对白、镜头类型、声音、灯光和场景布局同时可读。
- 用户需要先确认剧本拆分、每段时长、每个镜头的内容，再生成视频。
- 角色/场景一致性重要，且需要多个局部图共同说明。
- 外部图像模型生成整张复杂信息图时容易乱码、错字、排版失控。
...
这里的“脚本”是确定性排版工具，可以是 Python/Pillow、HTML/CSS 截图、Canvas、Figma 自动化或其他本地工具。它不属于 `seedance2` CLI 的 API 调用层；CLI 只登记最终故事板图片、`storyboard-prompt.txt`、`video-prompt.txt` 和确认记录。
...
- 如果同一长镜头因为模型时长上限必须跨段，在段落表里标记为同镜头延续，并进入 `multi-segment-continuity.md` 选择 `extend` 或尾帧硬衔接。
...
原子图片里尽量少放小字。CUT 编号、时间码、对白和镜头说明交给脚本绘制到最终故事板上。
...
- 所有中文文字、CUT 编号、时间码、对白和镜头说明由脚本绘制，避免图像模型乱码。
...
确认后进入 `seedance-handoff.md`：

```bash
seedance2 storyboard add <project-id> --storyboard-id <storyboard-id> --image <composed-storyboard.png> --prompt <storyboard-prompt.txt> --video-prompt <video-prompt.txt>
seedance2 storyboard approve <project-id> <storyboard-id>
seedance2 generate --project <project-id> --storyboard <storyboard-id> --duration <seconds> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait
```
...
| Symptom | Fix |
| ------- | --- |
| 图像模型生成的小字乱码 | 改成原子图片无字，文字由脚本绘制 |
| 故事板像散图拼贴 | 在 composition manifest 里明确读图顺序、区域主次和统一色板 |
| 视频只读到单张 CUT | `video-prompt.txt` 明确 CUT 区决定镜头顺序，其他区只做约束 |
| 第二段不连续 | 回到 `multi-segment-continuity.md`，写上一段 `end_state` 并选择正确 `continuity_mode`；只有硬衔接才用尾帧 |
