...
```bash
python scripts/seedance2_video.py doctor
python scripts/seedance2_video.py doctor --skip-connectivity
```
...
```bash
python scripts/seedance2_video.py config path
python scripts/seedance2_video.py config list
python scripts/seedance2_video.py config get model
python scripts/seedance2_video.py config set default_ratio 9:16
python scripts/seedance2_video.py config unset default_watermark
```

创建命令省略 `--model`、`--resolution`、`--ratio`、`--duration`、`--generate-audio`、`--watermark` 时，会使用配置文件中的默认值；`config set` 会按键名保存布尔值和整数，不把 `false` / `8` 错存成字符串。

配置文件位置（`SEEDANCE2_CONFIG_DIR` 可覆盖）：

| OS | Path |
|----|------|
| POSIX | `${XDG_CONFIG_HOME:-~/.config}/seedance2/config.json` |
| Windows | `%APPDATA%\seedance2\config.json` |
...
|-----------|------------------|
| 创建项目 | `python scripts/seedance2_video.py project create "用户原始想法"` |
| 检索资产 | `python scripts/seedance2_video.py asset search [--query ...] [--tag ...] [--type character|scene]` |
| 查看资产 | `python scripts/seedance2_video.py asset show <project_id> <asset_id>` |
| 复用资产 | `python scripts/seedance2_video.py asset reuse <target_project_id> <source_project_id> <source_asset_id>` |
| 登记资产 | `python scripts/seedance2_video.py asset add <project_id> --type character|scene --name ... --file image.png --tag ... --role ...` |
...
| 项目化生成 | `python scripts/seedance2_video.py generate --project <project_id> --storyboard <storyboard_id> [--reference-image <character-card>] [--reference-image <scene-background-card>] [--wait]` |
| 创建 (text-to-video) | `python scripts/seedance2_video.py text-to-video (--prompt "..." | --prompt-file prompt.txt) [--web-search] [--wait] [--download outputs]` |
| 创建 (first frame) | `python scripts/seedance2_video.py first-frame --first-frame frame.png [--prompt "..."] [--wait]` |
| 创建 (first+last) | `python scripts/seedance2_video.py first-last --first-frame start.png --last-frame end.png [--prompt "..."] [--wait]` |
| 创建 (omni) | `python scripts/seedance2_video.py omni [--prompt "..."] [--reference-image ...] [--reference-video https://...] [--reference-audio ...]` |
| 创建 (edit-intent) | `python scripts/seedance2_video.py edit --reference-video https://... [--prompt "..."] [--reference-image hint.png]` |
| 创建 (extend) | `python scripts/seedance2_video.py extend --reference-video https://... [--prompt "..."]` |
| 查询 | `python scripts/seedance2_video.py status <task_id>` |
| 等待 + 下载 | `python scripts/seedance2_video.py wait <task_id> --download outputs` |
| 列表 | `python scripts/seedance2_video.py list [--page-size 10] [--status running]` |
| 取消/删除 | `python scripts/seedance2_video.py delete <task_id>` |
| 生成历史 | `python scripts/seedance2_video.py history list <project_id>` |
| Callback receiver | `python scripts/seedance2_video.py callback-server --port 8787 --out-dir _work/seedance_callbacks` |
| Callback smoke test | `python scripts/seedance2_video.py callback-smoke http://127.0.0.1:8787/callback` |
...
python scripts/seedance2_video.py asset search --query "小红帽"
python scripts/seedance2_video.py asset search --type character --tag fairy-tale
python scripts/seedance2_video.py asset list proj-redhood --query "小红帽"
python scripts/seedance2_video.py asset show proj-redhood char-redhood
```

跨项目命中候选后，先让用户确认复用哪一项。长期复用到当前项目时用 `asset reuse`，它会把本地素材复制到目标项目资产目录，或登记同一个外部 source，并写入 `reused_from` 记录来源项目、来源资产、来源路径和 hash：

```bash
python scripts/seedance2_video.py asset reuse proj-new-ad proj-redhood char-redhood \
...
```bash
python scripts/seedance2_video.py project create "小红帽走向森林小屋" \
  --project-id proj-redhood --title "小红帽测试" --ratio 9:16 --duration 4

python scripts/seedance2_video.py asset add proj-redhood \
...
python scripts/seedance2_video.py script set proj-redhood --file script.md
python scripts/seedance2_video.py style set proj-redhood --file style.md
```

故事板流程不调用图片模型，只负责生成规划脚手架、登记故事板图片和确认状态。需要人物设定图、场景设定图或故事板母图时，Skill 先读 `visual-assets.md` 写图像提示词；九宫格、四栏、8 镜头或制作设定板只作为故事板母图的版式选项。故事板图应为 4K 或更高，`storyboard-prompt.txt` 保存完整绘图提示词，`video-prompt.txt` 保存简短读图指令。CLI 只接收生成后的图片路径或 URL：

```bash
python scripts/seedance2_video.py storyboard plan proj-redhood --layout 3x3

python scripts/seedance2_video.py storyboard add proj-redhood \
  --storyboard-id sb-main \
  --image storyboard.png \
  --prompt storyboard-prompt.txt \
  --video-prompt video-prompt.txt

python scripts/seedance2_video.py storyboard approve proj-redhood sb-main
```

从已确认故事板生成：

```bash
python scripts/seedance2_video.py generate \
...
```bash
python scripts/seedance2_video.py first-frame \
...
```bash
python scripts/seedance2_video.py project list
python scripts/seedance2_video.py project show proj-redhood
python scripts/seedance2_video.py storyboard show proj-redhood sb-main
python scripts/seedance2_video.py history list proj-redhood
python scripts/seedance2_video.py history show proj-redhood <run-id>
```
...
```bash
python scripts/seedance2_video.py text-to-video \
...
```bash
python scripts/seedance2_video.py first-last \
...
```bash
python scripts/seedance2_video.py omni \
...
```bash
python scripts/seedance2_video.py edit \
...
```bash
python scripts/seedance2_video.py extend \
...
```bash
python scripts/seedance2_video.py status <task_id>
python scripts/seedance2_video.py wait <task_id> --download outputs
python scripts/seedance2_video.py list --status running --page-size 20
python scripts/seedance2_video.py delete <task_id>
```
...
- `seedance2 doctor` 默认会做一次 `GET /contents/generations/tasks?page_size=1` 联通检查；离线核对加 `--skip-connectivity`。
