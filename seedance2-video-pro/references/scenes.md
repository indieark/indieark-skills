...
> Pairs with: `methods/README.md` 选择场记板或分镜；`director.md` 把场景裁剪成 prompt 字段。
...
Recommended method: `methods/storyboard.md` 用于卖点递进；素材/参数强约束时降级 `methods/clapperboard.md`。
...
Recommended method: `methods/storyboard.md`，但 5-15 秒内只保留一个事件或转折。
...
Recommended method: `methods/clapperboard.md` 用于主体一致性和动作路径控场；复杂舞蹈可局部借用 `methods/storyboard.md` 分拍。
...
Recommended method: `methods/clapperboard.md` 或不加载方法；避免无必要分镜。
...
Recommended method: `methods/storyboard.md`，首拍必须承担钩子。
...
Recommended method: `methods/clapperboard.md`，重点写清保留 / 改变 / 延续边界。
...
Recommended method: `methods/clapperboard.md` 用于素材角色控场；如果素材服务于多节拍叙事，再选 `methods/storyboard.md`。
