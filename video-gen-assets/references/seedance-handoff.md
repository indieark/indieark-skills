...
> Avoid: 还没有生成视觉资产提示词；先读对应的 `character-sheet.md`、`scene-sheet.md`、`../../video-gen-storyboard/references/storyboard-board.md`，再按需读 `../../video-gen-storyboard/references/grid-storyboard.md` 或 `../../video-gen-storyboard/references/four-column-storyboard.md`。
> Pairs with: `../../video-gen-storyboard/references/visual-storyboard.md` 管确认门和生成历史；`../../video-gen-pro/references/cli.md` 给完整命令；`../../video-gen-pro/references/completion.md` 规范最终汇报。

本文件把外部图像工具生成的视觉资产交接给 `videogen` CLI。CLI 不生成图片，只登记图片、提示词、确认记录和视频生成证据。
...
```bash
videogen project create "<用户原始想法或项目标题>" --project-id <project-id>
```

登记人物图：

```bash
videogen asset add <project-id> --type character --name <角色名> --file <character.png> --purpose "<主角外貌锁定 / 三视图 master / 反派设定>"
```

登记场景图：

```bash
videogen asset add <project-id> --type scene --name <场景名> --file <scene.png> --purpose "<主场景空间锁定 / 产品广告桌面场景 / SC01 正面角度>"
```

保存剧本和风格规则：

```bash
videogen script set <project-id> --file <script.md>
videogen style set <project-id> --file <style.md>
```
...
这条“短 prompt”只适用于已有故事板母图的路径。没有故事板时，回到 `../../video-gen-director/references/director-method.md` 的详细视频生成提示词结构，按每个分镜写清视觉内容、镜头参数、音频轨道、说话人声、字幕/标题文字和转场；不要用故事板路径的极简读图 prompt 代替详细分镜 prompt。
...
```bash
videogen storyboard add <project-id> --storyboard-id <storyboard-id> --image <storyboard.png> --prompt <storyboard-prompt.txt> --video-prompt <video-prompt.txt>
```
...
```bash
videogen generate --project <project-id> --storyboard <storyboard-id> --duration <seconds> --ratio <ratio> --generate-audio <true|false> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait
```

需要下载：

```bash
videogen generate --project <project-id> --storyboard <storyboard-id> --duration <seconds> --ratio <ratio> --generate-audio <true|false> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait --download <output-dir>
```
...
```bash
videogen generate --project <project-id> --storyboard sb-sd02 --duration <seconds> --reference-image <character-card.png> --reference-image <scene-background-card.png> --wait
```

只有需要画面硬衔接时，生成当前段才需要保留尾帧，并把结果尾帧用于下一段 `first-frame` 或 `first-last`：

```bash
videogen generate --project <project-id> --storyboard sb-sd01 --duration <seconds> --return-last-frame --wait
videogen first-frame --first-frame sd01-last-frame.png --prompt-file sd02-video-prompt.txt --duration <seconds> --wait
```

如果下一段只是延长上一段同一镜头：

```bash
videogen extend --reference-video <sd01-video-url> --prompt-file sd02-extension-prompt.txt --duration <seconds> --wait
```

提交前不确定时先 dry run：

```bash
videogen generate --project <project-id> --storyboard <storyboard-id> --duration <seconds> --reference-image <character-card.png> --reference-image <scene-background-card.png> --dry-run-payload
```
...
最终回复按 `../../video-gen-pro/references/completion.md`，并额外包含：
...
| 产品变形 | 故事板没有产品细节锁定 | 回到 `../../video-gen-storyboard/references/storyboard-board.md` 加产品细节区，必要时参考 `../../video-gen-storyboard/references/four-column-storyboard.md` |
